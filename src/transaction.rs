...
#[derive(Clone, PartialEq, Debug)]
pub struct CloseOrderFillTransaction {
    pub id: TransactionId,
    pub open_id: TransactionId,
    pub time: DateTime<Utc>,
...
#[derive(Clone, PartialEq, Debug)]
pub enum SimpleTransaction {
    OpenOrderFill(OpenOrderFillTransaction),
    CloseOrderFill(CloseOrderFillTransaction),
...
impl Transaction for SimpleTransaction {
    fn id(&self) -> TransactionId {
        match self {
            SimpleTransaction::OpenOrderFill(t) => t.id,
            SimpleTransaction::CloseOrderFill(t) => t.id,
            SimpleTransaction::Other(t) => t.id,
        }
    }

    fn time(&self) -> DateTime<Utc> {
        match self {
            SimpleTransaction::OpenOrderFill(t) => t.time,
            SimpleTransaction::CloseOrderFill(t) => t.time,
            SimpleTransaction::Other(t) => t.time,
        }
    }
}
